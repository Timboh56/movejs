...
exports.Region = var Region =  function(properties){
	...
	this.x = properties['x'],
		...
		this.fill_color = properties['fillColor'],
		this.label = properties['label'],
		this.moveFunction = properties['moveFunction'];
}
...
exports.Move = var Move = (function($) {
	...
	function Move(camera_id, canvas_id, blend_id, regions, properties) {
		this.DEFAULT_DISPLAY_CANVAS = 'none',
			this.DEFAULT_INTERVAL_AMOUNT = 10,
			this.DEFAULT_THRESHOLD = 0x33;

		var self = this,
			properties = properties || {};


		this.thresholdAmount = properties['thresholdAmount'] || this.DEFAULT_THRESHOLD,
		...
		// apply properties
		this.stream.style.display = properties.display_canvas || this.DEFAULT_DISPLAY_CANVAS,
			this.cnvs.style.display = properties.display_canvas || this.DEFAULT_DISPLAY_CANVAS,
			this.interval = properties.intervalAmount || this.DEFAULT_INTERVAL_AMOUNT;
	...
	Move.prototype = {
		...
		difference: function(target,data1,data2) {
			...
			while (i  < end) {
				...
				if ((val = this.threshold(avg_diff,this.thresholdAmount)) == 0xFF) {
